...
var app1 = new Vue({
    ...
    mounted: function(){
        ...
        uibuilder.onChange('msg', function(msg){
            ...
            if (msg.payload.co === "animacja") {
				const video = document.getElementById('video');
                app1.isonh = msg.payload.state
                if (msg.payload.state == "hidden") {
                    video.pause();
                    video.style.visibility = "hidden";
				} else {
					const scene = msg.payload.scene;
					const newBackground = msg.payload.background;
						//console.log("new= " + newBackground);
					video.src = scene;
					video.load();
					video.playbackRate = predkosc;
                    video.currentTime = 0;
                    video.play();
                    video.style.visibility = "visible";
					//video.addEventListener('play', () => {
					//	var div = document.getElementById("floorplan");
					//	if (isBackgroundDifferent(div, newBackground)) {
					//	console.log("zmiana");
					//	var elements = document.getElementsByClassName("elem");
					//	for (var i = 0; i < elements.length; i++) {
					//		elements[i].style.visibility = "hidden";
					//	}
					//div.style.backgroundImage = "url('./images/" + newBackground + "')";
					//				}
					//    });
					video.addEventListener('ended', () => {
                    video.style.visibility = "hidden";
                    uibuilder.send({ payload: "ended" });
});
				}
				
				//function isBackgroundDifferent(element, newBackgroundImage) {
				//	var style = getComputedStyle(element);
				//	var currentBackgroundImage = style.getPropertyValue("background-image");
					
				//	var currentBackgroundImageS = currentBackgroundImage.substring(currentBackgroundImage.lastIndexOf("/") + 1, currentBackgroundImage.length - 2);
					
					
				//	console.log("old= " + currentBackgroundImageS);
					
				//	return currentBackgroundImageS !== newBackgroundImage;
					//}
			} else {
				...
				const device = msg.payload.co;
				const state = msg.payload.state;
				document.getElementById(device).style.visibility = state;
			}
